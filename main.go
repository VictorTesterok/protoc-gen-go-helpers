...
import (
	...
	"github.com/VictorTesterok/protoc-gen-go-helpers/common"
)
...
func generateHelpers(gen *protogen.Plugin, file *protogen.File) *protogen.GeneratedFile {
	...
	for _, msg := range file.Messages {
		...
		if strings.Contains(string(msg.Comments.Trailing), "@parser:\"list\"") {
			...
			g.P("}")
			g.P()
			g.P("func (x *", msg.GoIdent, ") GetPostgresqlOptions() []interface{} {")
			g.P("var options []interface{}{}")
			if strings.Contains(string(msg.Comments.Trailing), "paging:true") {
				g.P("var limit int64 = 20")
				g.P("if x.Limit > 0 {")
				g.P("limit = x.Limit")
				g.P("}")
				g.P("options.append(&options, limit)")
				g.P("Options.append(&options, x.Skip)")
			}
			g.P("return options")
			g.P("}")
		}
